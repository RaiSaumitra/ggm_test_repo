...
def create_system(flowsheet=None, inf_kwargs={}, asm_kwargs={}, init_conds={}, lifetime=100, discount_rate=0.1,
  ...
  # 'O1': 98.96749331543272}
    ...
    PC = su.PrimaryClarifier(ID='PC', ins = [dom_ww, 
                                             # ind_ww, 
                                             eff_GT, eff_MT, eff_DU], 
                      outs = ['sludge_PC', 'effluent_PC'], isdynamic=True, 
                      init_with='WasteStream', thickener_perc= 2, 
...
if __name__ == '__main__':
    ...
    SRT = numerator_tank/denominator
    ...
    Microorganisms_based_on_volume = V_ae*(fs.ws1.get_VSS() + fs.ws3.get_VSS() + fs.ws5.get_VSS() + \
                                            fs.ws7.get_VSS() + fs.ws9.get_VSS() + fs.ws11.get_VSS())  
    # mg/L * m3 
        
    Microorganisms_based_on_flow = fs.ws1.F_vol*(fs.ws1.get_VSS() + fs.ws3.get_VSS() + fs.ws5.get_VSS() + \
                                            fs.ws7.get_VSS() + fs.ws9.get_VSS() + fs.ws11.get_VSS())  
    # mg/L * m3/hr 
        
    Food_w_RAS = fs.RAS.BOD*fs.RAS.F_vol + fs.effluent_PC.BOD*fs.effluent_PC.F_vol # mg/L * m3/hr
    
    Food = fs.effluent_PC.BOD*fs.effluent_PC.F_vol # mg/L * m3/hr
    
    print(f'Food/Microorganisms_based_on_volume = {Food/Microorganisms_based_on_volume}\n')
    
    # print(f'Food/Microorganisms_based_on_flow = {Food/Microorganisms_based_on_flow}\n')
    
    print(f'Food_w_RAS/Microorganisms_based_on_volume = {Food_w_RAS/Microorganisms_based_on_volume}\n')
    
    # print(f'Food_w_RAS/Microorganisms_based_on_flow = {Food_w_RAS/Microorganisms_based_on_flow}\n')
...
# ------------------------------------------------------------------------------------------------------------------
    ...
    fig, axis = fs.effluent.scope.plot_time_series(('S_S', 'X_S', 'S_NH', 'S_NO', 'X_I')) 
    fig